...
namespace LusoHealthClient.Server.Controllers
{
	...
	public class ProfileController : ControllerBase
	{
		...
		public async Task<ActionResult> UpdatePicture(UpdatePictureDto model)
		{
			var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
			...
			try
			{
				if (!string.IsNullOrEmpty(model.NewPictureUrl))
				{
					
					user.ProfilePicPath = model.NewPictureUrl;
					...
					if (result.Succeeded)
					{
						...
						var fileName = Path.GetFileName(new Uri(model.NewPictureUrl).LocalPath);
				...
				return BadRequest("O URL da nova imagem está vazio.");
			}
			catch (Exception ex)
			{
